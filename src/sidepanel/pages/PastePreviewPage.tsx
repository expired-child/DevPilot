...
export function PastePreviewPage({ item, targetFields, targetTitle, onBack, onConfirm }: Props) {
  ...
  const plan = useMemo(
    () => buildFillPlan(item, targetFields, { variables, overrides: uniqueOverrides }),
    [item, targetFields, variables, uniqueOverrides],
  );
  ...
  const canFill = plan.assignments.length > 0 && plan.missingVariables.length === 0 && uniqueValidation.valid;
  ...
  return (
    <>
      ...
      {(item.uniqueFieldKeys?.length ?? 0) > 0 && <section className="input-section"><h2>唯一字段</h2>{item.uniqueFieldKeys?.map((key) => {
        ...
        return <label className="field-label" key={key}>{field.label || field.name || key}<input value={uniqueOverrides[key] ?? field.value} onChange={(event) => setUniqueOverrides((current) => ({ ...current, [key]: event.target.value }))} /><small>原值：{field.value}</small></label>;
